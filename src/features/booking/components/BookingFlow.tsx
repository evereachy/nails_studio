...
import { useBookingStore, type BookingStep } from "../store/useBookingStore";
import { useAvailabilityStore } from "../store/useAvailabilityStore";
import { StepService } from "../steps/StepService";
import { StepWhen } from "../steps/StepWhen";
import { StepContact } from "../steps/StepContact";
import { StepDone } from "../steps/StepDone";
import { formatDuration, formatPrice, isValidPhone } from "@/lib/utils/format";
import { exceedsWorkday, summarize } from "@/features/booking/selection";
...
export function BookingFlow({ className }: { className?: string }) {
  ...
  const rescheduleId = useBookingStore((s) => s.rescheduleId);
    ...
    <div className={cn("flex min-h-0 flex-col", className)}>
      ...
      {rescheduleId && step < 4 && (
        <div className="mx-5 mb-3 flex items-center justify-between rounded-control bg-amber-500/10 px-4 py-2.5 text-xs font-medium text-amber-600 dark:text-amber-400">
          <span>
            ✏️ Перенос записи <strong>#{rescheduleId}</strong>
          </span>
